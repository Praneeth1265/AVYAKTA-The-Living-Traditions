...
export async function GET(
  ...
  try {
    ...
    const result = await retryWithBackoff(async () =>
      supabase.from("event_slug").select("*").eq("event_id", id),
    );
...
export async function POST(
  ...
  try {
    ...
    const { more_description, image_url } = body;
    ...
    const result = await retryWithBackoff(async () =>
      supabase.from("event_slug").insert([slugPayload]).select(),
    );
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Failed to create slug";
...
export async function PUT(
  ...
  try {
    await params;
    const body = await request.json();
    const { slugId, more_description, image_url } = body;
    ...
    if (more_description !== undefined)
      updatePayload.more_description = more_description?.trim();
