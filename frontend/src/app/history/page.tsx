    ...
    <main className="relative min-h-screen overflow-hidden" style={{ background: "#1c1c1c" }}>
      ...
      <section className="relative z-10 max-w-4xl mx-auto px-6 pb-32">
        ...
        <div
          ...
          style={{ background: "linear-gradient(to bottom, transparent 0%, #92791b 8%, #92791b 92%, transparent 100%)", opacity: 0.35 }}
          aria-hidden="true"
        />
...
function HeroSection() {
  ...
  useEffect(() => {
    setVis(true); // hero always visible
  }, []);
  ...
  return (
    <section ref={ref} className="relative z-10 pt-24 pb-20 text-center px-4">
      <p
        ...
        style={{ color: "#92791b", opacity: vis ? 1 : 0, transform: vis ? "translateY(0)" : "translateY(12px)" }}
      >
        Our Journey
      </p>
      <h1
...
function AnimatedDivider() {
  ...
  return (
    <div ref={ref} className="flex items-center justify-center gap-4 mt-10" aria-hidden="true">
      <div className="h-px w-32 transition-all duration-1000"
      ...
      <svg width="36" height="36" viewBox="0 0 40 40"
        ...
        style={{ opacity: vis ? 1 : 0, transform: vis ? "rotate(0deg) scale(1)" : "rotate(-45deg) scale(0.4)", transitionDelay: "300ms" }}>
      ...
      <div className="h-px w-32 transition-all duration-1000"
        style={{ background: "linear-gradient(to left, transparent, #92791b)", opacity: vis ? 1 : 0, transform: vis ? "scaleX(1)" : "scaleX(0)", transformOrigin: "left", transitionDelay: "150ms" }} />
    </div>
  );
}
...
function TimelineEntry({ milestone, index }: { milestone: HistoryMilestone; index: number }) {
  ...
  return (
    ...
    >
      {/* ── Left side ── */}
      <div className={`flex-1 w-full md:pr-12 ${isLeft ? "md:block" : "md:invisible"}`}>
        {isLeft && <EntryCard milestone={milestone} style={cardStyle} align="right" />}
      </div>
      ...
      {/* ── Right side ── */}
      <div className={`flex-1 w-full md:pl-12 ${!isLeft ? "md:block" : "md:invisible"}`}>
        {!isLeft && <EntryCard milestone={milestone} style={cardStyle} align="left" />}
      </div>
...
function EntryCard({ milestone, style, align }: {
      ...
      >
        ...
        <div className={`flex items-center gap-2 mb-3 ${align === "right" ? "justify-end" : "justify-start"}`}>
          ...
          <svg width="16" height="16" viewBox="0 0 20 20" fill="none" aria-hidden="true">
            <ellipse cx="10" cy="15" rx="6" ry="2.5" fill={milestone.is_ira_milestone ? "#1b5e3b" : "#92791b"} opacity="0.3" />
            <path d="M6.5 15 Q10 11 13.5 15" stroke={milestone.is_ira_milestone ? "#1b5e3b" : "#c9a84c"} strokeWidth="1.2" fill="none" />
            <path d="M10 11 Q10.5 7 10 4" stroke="#c9a84c" strokeWidth="1.2" strokeLinecap="round" />
            <ellipse cx="10" cy="3.5" rx="1.2" ry="1.8" fill="#c9a84c" opacity="0.9" />
          </svg>
          <span
            className="text-xs font-bold tracking-[0.25em] uppercase px-3 py-1 rounded-full"
            style={{
              background: milestone.is_ira_milestone ? "rgba(27,94,59,0.15)" : "rgba(146,121,27,0.12)",
          ...
          >
            {milestone.year}{milestone.is_ira_milestone && " · IRA"}
          </span>
        </div>
