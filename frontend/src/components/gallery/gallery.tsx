...
const TAGS = ["All", "Dance", "Music", "Art", "Theatre", "Culture", "Opening", "Finale"];
...
// ─── Ticker words ─────────────────────────────────────────────────────────────
const TICKER = [
  "BHARATANATYAM", "✦", "CARNATIC", "✦", "STREET ART", "✦",
  "THEATRE", "✦", "PERCUSSION", "✦", "FOLK DANCE", "✦",
  "SCULPTURE", "✦", "POETRY", "✦", "KALAVAIBHAVA", "✦",
];
...
export default function GalleryPage() {
  ...
  return (
    <>
      ...
      <div className="noise" style={{ position: "relative", minHeight: "100vh", background: "var(--bg)", zIndex: 1 }}>
        ...
        <nav className="nav" style={{
          ...
          <div style={{ maxWidth: 480, margin: "0 auto", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 20, filter: "drop-shadow(0 0 8px rgba(201,168,76,0.7))", animation: "float 3s ease-in-out infinite" }}>🪔</span>
              <span className="font-display" style={{ fontSize: 11, color: "var(--gold)", letterSpacing: "0.15em" }}>KV 2025</span>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <span className="font-heading" style={{ fontSize: 10, color: "var(--olive)", letterSpacing: "0.1em", padding: "6px 12px", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 100 }}>SHARE</span>
              <span style={{ fontSize: 18, cursor: "pointer", color: "var(--olive)", transition: "color 0.2s" }}>♥</span>
            </div>
          </div>
        </nav>

        {/* ── Hero ── */}
        <section style={{ position: "relative", overflow: "hidden", padding: "48px 16px 32px" }}>
          ...
          <div style={{ maxWidth: 480, margin: "0 auto", position: "relative" }}>
            ...
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
              <div style={{
                ...
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--gold)", boxShadow: "0 0 8px var(--gold)", animation: "float 2s ease-in-out infinite" }} />
                <span className="font-heading" style={{ fontSize: 9, letterSpacing: "0.25em", color: "var(--gold)", textTransform: "uppercase" }}>Photo Gallery</span>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--gold)", boxShadow: "0 0 8px var(--gold)", animation: "float 2s ease-in-out infinite 1s" }} />
              </div>
            </div>

            {/* Main title */}
            <div className="hero-title" style={{ textAlign: "center", marginBottom: 12 }}>
              <h1 className="font-display glitch hero-title-text" style={{
                fontSize: "2.4rem",
                lineHeight: 1.1,
                color: "var(--gold)",
            ...
            {/* Tagline */}
            <div className="hero-sub" style={{ textAlign: "center", marginBottom: 28 }}>
              <p style={{ fontStyle: "italic", color: "rgba(245,240,232,0.65)", fontSize: 15 }}>
            ...
            <div className="hero-stats" style={{
              ...
              {STATS.map((s, i) => (
                <div key={s.label} style={{
                  background: "linear-gradient(135deg, rgba(28,28,28,0.8), rgba(14,13,11,0.9))",
                ...
                >
                  ...
                  <div className="font-display" style={{ fontSize: 16, color: "var(--gold)", lineHeight: 1 }}>{s.value}</div>
                  <div className="font-heading" style={{ fontSize: 8, color: "var(--olive)", letterSpacing: "0.1em", marginTop: 3 }}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="ticker-wrap" style={{
          background: "linear-gradient(135deg, var(--bronze), var(--gold), var(--bronze))",
          padding: "10px 0",
        ...
        {/* ── Filter pills ── */}
        <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 16px 20px", position: "relative", zIndex: 2 }}>
        ...
        {/* ── Gallery grid ── */}
        <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 16px 32px", position: "relative", zIndex: 2 }}>
          <div className="gallery-grid">
            {filtered.map((item) => {
              // Parse span classes
              const isWide = item.span.includes("col-span-2") || item.span.includes("col-span-3");
              ...
              return (
                <div
                  ...
                  style={{
                    gridColumn: isWide3 ? "1 / -1" : isWide ? "span 2" : "span 1",
                ...
                >
                  ...
                  {/* Corner accents */}
                  <div style={{ position: "absolute", top: 8, left: 8, width: 16, height: 16, borderTop: `2px solid ${item.color}`, borderLeft: `2px solid ${item.color}`, borderRadius: "2px 0 0 0", opacity: 0.7 }} />
                  <div style={{ position: "absolute", bottom: 8, right: 8, width: 16, height: 16, borderBottom: `2px solid ${item.color}`, borderRight: `2px solid ${item.color}`, borderRadius: "0 0 2px 0", opacity: 0.7 }} />

                  <div className="card-inner">
                    <span style={{ filter: `drop-shadow(0 0 16px ${item.color})` }}>{item.placeholder}</span>
                  </div>

                  <div className="card-overlay">
                    <span className="card-tag" style={{ color: item.color, borderColor: item.color }}>
                      {item.tag}
                    </span>
          ...
          {filtered.length === 0 && (
            <div style={{ textAlign: "center", padding: 60, color: "var(--olive)" }}>
              <p className="font-heading" style={{ fontSize: 12, letterSpacing: "0.1em" }}>NO ITEMS IN THIS CATEGORY</p>
            </div>
          )}
        </div>

        {/* ── Highlight quote ── */}
        <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 16px 40px", position: "relative", zIndex: 2 }}>
          <div style={{
            background: "linear-gradient(135deg, rgba(139,26,26,0.2), rgba(100,18,18,0.1))",
            ...
            <div className="ornament" style={{ marginTop: 16 }}>
              <span className="font-heading" style={{ fontSize: 9, color: "var(--crimson)", letterSpacing: "0.2em" }}>
                KALAVAIBHAVA SPIRIT
              </span>
            </div>
          </div>
        ...
        <div className="ticker-wrap" style={{
          ...
          <div style={{ animation: "ticker 35s linear infinite reverse", display: "inline-block", whiteSpace: "nowrap" }}>
            {[...Array(20)].map((_, i) => (
              <span key={i} className="font-heading" style={{
                fontSize: 9, letterSpacing: "0.15em",
                color: "rgba(27,94,59,0.8)", margin: "0 20px",
              }}>
                {i % 3 === 0 ? "📸 CAPTURE THE MOMENT" : i % 3 === 1 ? "🎬 RELIVE THE MAGIC" : "✨ MEMORIES FOREVER"}
              </span>
            ))}
          </div>
        </div>
        ...
        {/* ── CTA ── */}
        <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 16px 48px", position: "relative", zIndex: 2 }}>
          <div className="ornament" style={{ marginBottom: 24 }}>
            <span className="font-heading" style={{ fontSize: 9, letterSpacing: "0.2em", color: "var(--olive)", whiteSpace: "nowrap" }}>
              REGISTER FOR 2025
            </span>
          </div>
          ...
          <button
            ...
            onMouseEnter={e => {
              ...
              (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 16px 50px rgba(201,168,76,0.4)";
            }}
            onMouseLeave={e => {
              (e.currentTarget as HTMLButtonElement).style.backgroundPosition = "0 0";
              (e.currentTarget as HTMLButtonElement).style.transform = "translateY(0)";
              (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 8px 40px rgba(201,168,76,0.25)";
            }}
          >
            ✦ BE PART OF KALAVAIBHAVA 2025 ✦
          </button>

          <p style={{ textAlign: "center", marginTop: 12, fontSize: 12, color: "rgba(115,121,85,0.6)", fontStyle: "italic" }}>
        ...
        <footer style={{
          ...
          <div style={{ fontSize: 24, marginBottom: 8, filter: "drop-shadow(0 0 8px rgba(201,168,76,0.6))" }}>🪔</div>
          <p className="font-display" style={{ fontSize: 9, color: "var(--bronze)", letterSpacing: "0.2em", marginBottom: 16 }}>
            KALAVAIBHAVA 2025
          </p>
          <div style={{ display: "flex", justifyContent: "center", gap: 24, marginBottom: 16 }}>
            {["About", "Gallery", "Schedule", "Contact"].map(l => (
              <span key={l} className="font-heading" style={{ fontSize: 9, color: "var(--olive)", letterSpacing: "0.1em", cursor: "pointer" }}>{l}</span>
            ))}
          </div>
          <p style={{ fontSize: 10, color: "rgba(28,28,28,0.5)" }}>© 2025 Kalavaibhava. All rights reserved.</p>
        </footer>
      </div>
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                background: "rgba(8,7,5,0.8)", border: "1px solid rgba(201,168,76,0.3)",
            ...
            {/* Media area */}
            <div className="lightbox-media" style={{
            ...
            <div style={{ padding: "20px 24px 24px" }}>
              ...
              <h3 className="font-heading" style={{ fontSize: 16, color: "var(--cream)", letterSpacing: "0.05em", marginBottom: 8 }}>
                {lightboxItem.label}
              </h3>
              <p style={{ fontSize: 13, color: "rgba(245,240,232,0.55)", lineHeight: 1.6 }}>
                A stunning moment captured at Kalavaibhava 2025 — celebrating the rich tapestry of Indian arts and culture.
              </p>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
                <button
                  onClick={() => {
                    const idx = GALLERY_ITEMS.findIndex(g => g.id === lightbox);
